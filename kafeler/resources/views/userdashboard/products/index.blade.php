            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    @foreach ($products as $product)
                        <tr>
                            <td>
                                <input type="checkbox" name="ids[]" value="{{ $product->id }}" class="bulk-checkbox">
                            </td>
                            ...
                            <td>
                                <label class="switch">
                                    <input type="checkbox" data-id="{{ $product->id }}" {{ $product->status ? 'checked' : '' }}>
                                    <span class="slider round"></span>
                                </label>
                            </td>
                            <td>
                                @if ($product->main_image)
                                    <img src="{{ asset('storage/product_main_images/' . $product->main_image) }}" alt="{{ $product->name }}" width="50" class="cursor-pointer" data-bs-toggle="modal" data-bs-target="#imageModal{{ $product->id }}">
                                @else
                                    Resim Yok
                                @endif
                            </td>
                            <td>
                                ...
                                <button type="button" class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#sliderModal{{ $product->id }}">
                                            ...
                                            <div class="swiper-wrapper">
                                                @foreach ($product->images as $image)
                                                    <div class="swiper-slide">
                                                        <img src="{{ asset('storage/product_slider_images/' . $image->image) }}" alt="{{ $product->name }}" class="img-fluid" style="max-height: 70vh; object-fit: contain;">
                                                    </div>
                                                @endforeach
                                            </div>
...
@section('scripts')
    ...
    <script>
        ...
        document.getElementById('bulkDeleteForm').addEventListener('submit', function(e) {
            ...
            Swal.fire({
                title: 'Emin misiniz?',
                text: "Seçilen ürünler silinecek!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#3085d6',
